...
const ClientLogoCarousel = () => {
  ...
  return (
    <div className="py-12 bg-background-500">
      <h2 className="text-2xl font-bold text-primary-700 mb-6 text-center animate-slide-down">
      ...
      <Slider {...settings} className="logo-carousel">
        {(galleryItems || []).map((item, index) => (
          ...
          >
            ...
            >
              {item ? (
                <div className="relative w-full h-full flex items-center justify-center">
                  <motion.img
                    src={item.imageUrl} // Use imageUrl from galleryItems
            ...
            <p className="mt-2 text-center text-sm text-neutral-700 font-medium">
              {item.description}
            </p>
          </div>
        ))}
